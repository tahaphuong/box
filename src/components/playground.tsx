...
export function Playground() {
    const { solution, stats } = useContext(MainContext) ?? { solution: null, stats: null };
        ...
        {stats && (
          <div>
            <div className="text-center mb-1">
              <div className="text-center mb-1">
                <strong>Run time:</strong> {stats.runtime?.toFixed(2) + 'ms'}
              </div>
              <strong>Number of boxes:</strong> {stats.numBox}
              {stats.numBoxImproved != null && (
                <span className="ml-2">
                  <strong>Boxes improved:</strong> {stats.numBoxImproved}
                </span>
              )}
                <br />
              {stats.score != null && (
              ...
              {stats.scoreImproved != null && (
                <span className="ml-2">
                  <strong>Improved score:</strong> {stats.scoreImproved}
                </span>
              )}
            </div>
          </div>
              ...
              >
                ...
                {box.rectangles.map((rect, idx) => (
                  <g key={idx}>
                    ...
                    <text
                      x={rect.x * scale + (rect.getWidth * scale) / 2}
                      y={rect.y * scale + (rect.getHeight * scale) / 2}
                      textAnchor="middle"
